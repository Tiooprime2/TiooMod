...
public class TiooGui extends Screen {
    ...
    public void render(DrawContext ctx, int mx, int my, float delta) {
        ...
        ctx.drawText(textRenderer, "v1.0",   winX+WIN_W-22, winY+5, C_MUTED, false);

        // Entries
        int y = winY + HDR_H + PAD - (int)scrOff;
        for (Entry e : entries) {
            if (y + BTN_H > winY+HDR_H && y < winY+WIN_H)
                renderBtn(ctx, e, winX+PAD, y, mx, my);
            y += BTN_H + PAD;
            if (e.exp) {
                int sh = subH(e);
                if (y > winY+HDR_H && y < winY+WIN_H)
                    renderSub(ctx, e, winX+PAD, y, mx, my);
                y += sh + PAD;
            }
        }
    ...
    private void renderBtn(DrawContext ctx, Entry e, int x, int y, int mx, int my) {
        ...
        int bx = x+w-24, by = y+BTN_H/2-4;
        fill(ctx, bx, by, bx+20, by+10, on ? col(20,60,25,200) : col(30,30,45,200));
        ctx.drawText(textRenderer, on?"ON":"OFF", bx+2, by+1, on ? C_GREEN : C_MUTED, false);
        ctx.drawText(textRenderer, e.exp?"▲":"▼", x+w-10, y+5, C_MUTED, false);
    }
        ...
        for (Entry e : entries) {
            ...
            if (e.exp) {
                ...
                if (mx>=winX+PAD && mx<=winX+WIN_W-PAD && my>=y && my<=y+sh) {
                    ...
                    if (rawH > 60) {
                        scrVel = 0;
                        e.subScr = Math.max(0, Math.min(e.subScr-v*6, rawH-60));
                    }
                    return true;
                }
                y += sh+PAD;
    ...
    @Override public boolean mouseReleased(double mx, double my, int btn) { drag=false; return super.mouseReleased(mx,my,btn); }
    ...
    private void fill(DrawContext c, int x1,int y1,int x2,int y2,int col) { c.fill(x1,y1,x2,y2,col); }
