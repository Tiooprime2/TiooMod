...
/**
 * TiooGui — Fixed blur dengan unscaledProjection seperti Argon
 *
 * ROOT CAUSE BLUR:
 * Argon pakai RenderUtils.unscaledProjection() sebelum render GUI
 * Ini ubah projection matrix ke framebuffer pixels (bukan scaled coords)
 * Tanpa ini semua render jadi blur/blur di HP karena scale factor != 1
 */
public class TiooGui extends Screen {
    ...
    // ─── Sizes (dalam FRAMEBUFFER pixels, bukan scaled) ───────────────────
    // Argon pakai unscaledProjection jadi semua koordinat = framebuffer pixels
    // Untuk HP biasanya scaleFactor = 2, jadi semua ukuran * 2
    private static final int WIN_W  = 390;  // 195 * 2
    private static final int WIN_H  = 520;  // 260 * 2
    private static final int HDR_H  = 36;   // 18  * 2
    private static final int BTN_H  = 76;   // 38  * 2
    private static final int PAD    = 8;    // 4   * 2
    private static final int SUB_H  = 30;   // 15  * 2

    // ─── State ────────────────────────────────────────────────────────────
    private int winX = 160, winY = 100;
    private boolean drag; int dx, dy;
    private double scrOff, scrVel;

    // Untuk restore projection matrix
    private VertexSorter savedVertexSorter;
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        fill(ctx, winX+WIN_W/4, winY, winX+WIN_W*3/4, winY+2, C_ACCENT);
        ctx.drawText(textRenderer, "✦ Tioo", winX+10, winY+10, C_TEXT, false);
        ctx.drawText(textRenderer, "v1.0",   winX+WIN_W-36, winY+10, C_MUTED, false);

        // Entries
        int y = winY + HDR_H + PAD - (int)scrOff;
        for (Entry e : entries) {
            if (y+BTN_H > winY+HDR_H && y < winY+WIN_H)
                renderBtn(ctx, e, winX+PAD, y, scaledMouseX, scaledMouseY);
            y += BTN_H + PAD;
            if (e.exp) {
                int sh = subH(e);
                if (y > winY+HDR_H && y < winY+WIN_H)
                    renderSub(ctx, e, winX+PAD, y, scaledMouseX, scaledMouseY);
                y += sh + PAD;
            }
        }
        ...
        // Scrollbar
        if (totH > visH) {
            int sbx = winX+WIN_W-6, sby = winY+HDR_H, sbh = visH;
            int th = Math.max(20, (int)(sbh*(float)visH/totH));
            int ty = sby + (int)((sbh-th)*(float)(scrOff/Math.max(1,totH-visH)));
            fill(ctx, sbx, sby, sbx+4, sby+sbh, C_SCRBG);
            fill(ctx, sbx, ty,  sbx+4, ty+th,   C_THUMB);
        }

        // Restore projection — wajib setelah unscaled!
        scaledProjection(mc);
    ...
    private void renderBtn(DrawContext ctx, Entry e, int x, int y, int mx, int my) {
        ...
        fill(ctx, x, y+6, x+4, y+BTN_H-6, on ? C_GREEN : C_MUTED);
        ctx.drawText(textRenderer, e.mod.getName(),        x+12, y+8,  C_TEXT,  false);
        ctx.drawText(textRenderer, e.mod.getDescription(), x+12, y+28, C_MUTED, false);
        int bx = x+w-44, by = y+BTN_H/2-8;
        fill(ctx, bx, by, bx+38, by+18, on ? col(20,60,25,200) : col(30,30,45,200));
        ctx.drawText(textRenderer, on?"ON":"OFF", bx+6, by+4, on ? C_GREEN : C_MUTED, false);
        ctx.drawText(textRenderer, e.exp?"▲":"▼", x+w-18, y+8, C_MUTED, false);
    }
    ...
    private void renderSub(DrawContext ctx, Entry e, int x, int y, int mx, int my) {
        ...
        int sy = y+4-(int)e.subScr;
        for (BS s : e.settings) {
            if (sy+SUB_H > y && sy < y+h) {
                if (mx>=x && mx<=x+w && my>=sy && my<=sy+SUB_H)
                    fill(ctx, x, sy, x+w, sy+SUB_H, col(255,255,255,15));
                ctx.drawText(textRenderer, s.label, x+8, sy+6, C_TEXT, false);
                ctx.drawText(textRenderer, s.val?"✓":"✗",
                    x+w-20, sy+6,
    ...
    // ─── Projection helpers (sama seperti Argon RenderUtils) ──────────────

    private void unscaledProjection(MinecraftClient mc) {
        savedVertexSorter = RenderSystem.getVertexSorting();
        RenderSystem.setProjectionMatrix(
            new Matrix4f().setOrtho(
                0, mc.getWindow().getFramebufferWidth(),
                mc.getWindow().getFramebufferHeight(), 0,
                1000, 21000),
            VertexSorter.BY_Z);
    }

    private void scaledProjection(MinecraftClient mc) {
        RenderSystem.setProjectionMatrix(
            new Matrix4f().setOrtho(
                0, (float)(mc.getWindow().getFramebufferWidth()  / mc.getWindow().getScaleFactor()),
                   (float)(mc.getWindow().getFramebufferHeight() / mc.getWindow().getScaleFactor()),
                0, 1000, 21000),
            savedVertexSorter != null ? savedVertexSorter : VertexSorter.BY_Z);
    }

    // ─── Mouse events — koordinat juga perlu di-scale ─────────────────────

    @Override
    public boolean mouseScrolled(double mx, double my, double h, double v) {
        MinecraftClient mc = MinecraftClient.getInstance();
        double smx = mx * mc.getWindow().getScaleFactor();
        double smy = my * mc.getWindow().getScaleFactor();
        ...
        for (Entry e : entries) {
            ...
            if (e.exp) {
                int sh = subH(e);
                if (smx>=winX+PAD && smx<=winX+WIN_W-PAD && smy>=y && smy<=y+sh) {
                    int rawH = e.settings.size()*SUB_H+8;
                    if (rawH > 120) {
                        scrVel = 0;
                        e.subScr = Math.max(0, Math.min(e.subScr-v*12, rawH-120));
                    }
                    return true;
                }
                y += sh + PAD;
        ...
        if (smx>=winX && smx<=winX+WIN_W && smy>=winY && smy<=winY+WIN_H) {
            if (totH() > WIN_H-HDR_H) scrVel -= v*16;
        }
    ...
    public boolean mouseClicked(double mx, double my, int btn) {
        MinecraftClient mc = MinecraftClient.getInstance();
        int smx = (int)(mx * mc.getWindow().getScaleFactor());
        int smy = (int)(my * mc.getWindow().getScaleFactor());

        if (smx>=winX && smx<=winX+WIN_W && smy>=winY && smy<=winY+HDR_H) {
            drag=true; dx=smx-winX; dy=smy-winY; return true;
        }
        int y = winY+HDR_H+PAD-(int)scrOff;
        for (Entry e : entries) {
            int x=winX+PAD, w=WIN_W-PAD*2;
            if (smx>=x && smx<=x+w && smy>=y && smy<=y+BTN_H) {
            ...
            if (e.exp) {
                ...
                for (BS s : e.settings) {
                    if (smx>=x+8 && smx<=x+w-8 && smy>=sy && smy<=sy+SUB_H) {
    ...
    public boolean mouseDragged(double mx, double my, int btn, double ddx, double ddy) {
        if (drag) {
            ...
            int smx = (int)(mx * mc.getWindow().getScaleFactor());
            int smy = (int)(my * mc.getWindow().getScaleFactor());
            winX = smx-dx; winY = smy-dy; return true;
        }
    ...
    private int subH(Entry e) { return Math.min(e.settings.size()*SUB_H+8, 160); }
    ...
    private void border(DrawContext c, int x,int y,int w,int h,int col) {
        c.fill(x,y,x+w,y+2,col); c.fill(x,y+h-2,x+w,y+h,col);
        c.fill(x,y,x+2,y+h,col); c.fill(x+w-2,y,x+w,y+h,col);
    }
